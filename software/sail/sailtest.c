...
#define DUTY_CYCLE 75        //motor PWM duty cycle as a percentage
#define SENSOR_THRESHOLD 400 //10 bit (max 1023) stop sensor trigger threshold
...
/* This Function enables all pins for motor driving and sensing:
...
void initADC(void) {
    ...
    ADCCTL0 = ADCSHT_2 | ADCON; //ADC on, timer set to 64 ticks per sample
...
void runMotor(int mode, int dutyCycle) {
    ...
    case 1:
        ...
        break;
    default:
        TB1CTL = MC_0; //turn off PWM timer
        TB1CCTL1 = OUTMOD_0;
        P2OUT = 0b0; //turn off pin
        break;
    }
}
...
void deploySail(void) {
    ...
    while (1) {
        ...
        if (ADCresult(-1) >= SENSOR_THRESHOLD) {
            runMotor(0,0);
            //break;
        }
    ...
    __bis_SR_register(LPM4_bits); //shutdown
}